...
| `patch_diffusion_worker_sleep_cache_clear` | Clears `PromptEmbedCache` before `DiffusionWorker.sleep()` executes `allocator.sleep()`, ensuring all cached GPU embeddings are freed before PyTorch memory cache release during colocated training | vllm-omni clears prompt-embedding caches in `DiffusionWorker.sleep()` natively |
