...
| `register_capture_flush` (`plugin.py`) | Flushes `_unirl_captures` into formatter `metadata["unirl"]` after postprocess. Stable 0.28 loads the general plugin in every stage process, so no child-process monkeypatch is needed | vllm-omni exposes the captured RL metadata natively |
...
| `patch_hv15_refiner_torch_linear_lora` | Pinned vllm-omni's diffusion LoRA manager replaces only vLLM linear classes. HV1.5 deliberately keeps the token-refiner projections as Diffusers `nn.Linear` layers, so configured refiner LoRA tensors are loaded but never applied to the rollout policy. Reuse the manager's PEFT target matcher to discover and wrap those ordinary linears with the same BF16 base-plus-adapter arithmetic | vllm-omni applies diffusion LoRA to HV1.5's ordinary token-refiner linears |
...
| `compat_hi3_lora` (module) | HI3's AR model returns `(expert_mapping, replacements)`, while vLLM 0.28's LoRA manager still iterates the result as a flat four-field mapping and fails at boot | vLLM consumes the mapping-plus-replacements tuple natively |
| `patch_moe_workspace_pool` (`compat_moe_workspace`) | vLLM's reusable MoE `WorkspaceManager` scratch buffer is allocated outside the `weights`/`kv_cache` CuMem pools, so worker sleep leaves it resident through the colocated train phase. Gives each DBO×lane workspace an independently replaceable CuMem pool while tagging all allocations `moe_workspace`; worker sleep then discards the scratch pages and UniRL's untagged wake remaps them. Self-skips when the allocator is absent | vLLM allocates the MoE workspace inside a sleep-managed pool itself |
| `patch_diffusion_worker_sleep_cache_clear` | With `omni_extra.enable_prompt_embed_cache` (or `OMNI_DIFFUSION_PROMPT_EMBED_CACHE=1`), `PromptEmbedCache` holds `encode_prompt` outputs allocated by the default torch allocator, outside the CuMem pools, so `allocator.sleep()` leaves them resident through the colocated train phase. Calls the runner's `clear_prompt_embed_cache()` before the original `sleep`, whose `empty_cache()` then returns the pages. No-op when the cache is off. Verified still absent in v0.28.0 and v0.29.0rc1 | vllm-omni clears the prompt-embedding cache in `DiffusionWorker.sleep()` itself |
